 ...
 * @apiSuccess (200) {Array} data.url 下载的地址（相对路径）
 * @apiSuccess (200) {Array} data.version_1 第一级：重大重构
 * @apiSuccess (200) {Number} data.version_2 第二级：重大功能改进
 * @apiSuccess (200) {String} data.version_3  第三极：小升级或者BUG修复
 * @apiSuccess (200) {String} data.type 类型:[android,ios,pc]
 * @apiSuccess (200) {Number} data.title  更新的标题
 * @apiSuccess (200) {String} data.description 更新版本描述
 * @apiSuccess (200) {Array} data.createTime  创建时间
...
indexRouter.post('/addVersion', (ctx: Context, next: Next) => {
    return multerUtil.uploadAPK(ctx, next).then(async () => {
        ...
        const data: any = (await VersionUpgrade.instance.save({
            title,
            description,
            type,
            version_1,
        ...
        data.url = multerUtil.getAPKUrlPath(type, version_1, version_2, version_3);
 ...
 * @apiSuccess (200) {String} data.version_3  第三极：小升级或者BUG修复
 * @apiSuccess (200) {String} data.type 类型:[android,ios,pc]
 * @apiSuccess (200) {Number} data.title  更新标题
 * @apiSuccess (200) {String} data.description 更新版本描述
 * @apiSuccess (200) {Array} data.createTime  创建时间
 * @apiSuccessExample {json} Success-Response:
 *  {
...
indexRouter.get('/latestVersion', async (ctx: Context, next: Next) => {
    ...
    const data = await VersionUpgrade.instance.findOne({type}).sort({createTime: -1}).limit(1);
