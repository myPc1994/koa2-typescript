...
import {JsTool} from "../tools/JsTool";
import {MulterTool} from "../tools/MulterTool";
...
indexRouter.post('/addExcel2NucleicAcid', async (ctx: Context, next: Next) => {
    const config: IMulterUtil = {
        ...
        filename: function (req: any, file: any, cb: Function) {
            const name = moment().format("YYYY_YY_MM_DD_HH_mm") + JsTool.getType(file.originalname);
            cb(null, name)
        }
    }
    return MulterTool.getMulter("nucleate", ctx, next, config).then(async () => {
